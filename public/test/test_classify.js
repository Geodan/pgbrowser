...
describe('classify', ()=>{
    ...
    afterEach(()=>{
        document.body.removeChild(ramp);
    });
    it('testing classifications', ()=>{
        let classes = classify(stats, classCount, 'mostfrequent', schemes[1].colors);
        assert.equal(classes[5].label, 25, 'label for class[5] should be 25');
        classes = classify(stats, 11, 'interval', schemes[1].colors);
        assert.equal(classes[5].label, '2120 - 2540', "label for class[5] should be '2120 - 2540'");
    });
})